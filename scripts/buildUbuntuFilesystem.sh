...
# use Ubuntu 18.04 (Long Term Support) if no suite is specified
if [ "$PRAWNOS_SUITE" = "" ]
then
    PRAWNOS_SUITE=bionic
fi

# create a 4GB image with the Chrome OS partition layout
create_image PrawnOS-${PRAWNOS_SUITE}-Alpha-c201-libre-2GB.img-BASE $outdev 50M 80 $outmnt

# use default debootstrap mirror if none is specified
if [ "$PRAWNOS_DEBOOTSTRAP_MIRROR" = "" ]
then
    PRAWNOS_DEBOOTSTRAP_MIRROR=http://ports.ubuntu.com/ubuntu-ports
fi

# install Debian on it
export DEBIAN_FRONTEND=noninteractive
qemu-debootstrap --arch armhf $PRAWNOS_SUITE --include locales,init --keyring=$build_resources/ubuntu-archive-keyring.gpg $outmnt $PRAWNOS_DEBOOTSTRAP_MIRROR
...
cp $build_resources/ubuntu.list $outmnt/etc/apt/sources.list
...
# initscripts does not exist in ubuntu, util-linux sysvinit-utils referred instead 
chroot $outmnt apt update
chroot $outmnt apt upgrade -y
chroot $outmnt apt install -y util-linux sysvinit-utils udev kmod net-tools inetutils-ping traceroute iproute2 isc-dhcp-client wpasupplicant iw alsa-utils cgpt vim-tiny less psmisc netcat-openbsd ca-certificates bzip2 xz-utils ifupdown nano apt-utils git kpartx gdisk parted rsync
...
#Package 'acpi-support' has no installation candidate
#Unable to locate package xfce4-screensaver
chroot $outmnt apt-get install -y -d xorg lightdm tasksel dpkg librsvg2-common xorg xserver-xorg-input-libinput alsa-utils anacron avahi-daemon eject iw libnss-mdns xdg-utils lxqt crda xfce4 dbus-user-session system-config-printer tango-icon-theme xfce4-power-manager xfce4-terminal xfce4-goodies mousepad vlc libutempter0 xterm numix-gtk-theme dconf-cli dconf-editor plank network-manager-gnome network-manager-openvpn network-manager-openvpn-gnome dtrx emacs25 accountsservice sudo pavucontrol-qt

#Experimental, download graphical environment to prebuild with
chroot $outmnt apt-get install -y -d gdm3 gnome-terminal nautilus network-manager-gnome network-manager-openvpn network-manager-openvpn-gnome gnome-session dbus-user-session accountsservice gnome-js-common gnome-shell-extensions fonts-cantarell 
